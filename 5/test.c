...
static void test_msg_delete(struct test_msg *msg) { free(msg); }

static void chat_message_extract_id(struct chat_message *msg, int *cli_id,
                                    int *msg_id) {
...
static void server_consume_events(struct chat_server *s) {
  ...
  while ((rc = chat_server_update(s, 0)) == 0) {
    printf("server updating: %d\n", rc);
  };
  printf("server updated: %d\n", rc);
  unit_fail_if(rc != CHAT_ERR_TIMEOUT);
}

static struct chat_message *server_pop_next_blocking_from(
    struct chat_server *s, struct chat_client *c) {
  struct chat_message *msg;
  while ((msg = chat_server_pop_next(s)) == NULL) {
    chat_client_update(c, 0);
    chat_server_update(s, 0);
    printf("server_pop_next_blocking_from...\n");
  }
  return msg;
}
...
static void client_consume_events(struct chat_client *c) {
  ...
  while ((rc = chat_client_update(c, 0)) == 0) {
    printf("client updating: %d\n", rc);
  };
  printf("client updated: %d\n", rc);
  unit_fail_if(rc != CHAT_ERR_TIMEOUT);
}

static struct chat_message *client_pop_next_blocking(struct chat_client *c,
                                                     struct chat_server *s) {
  struct chat_message *msg;
  while ((msg = chat_client_pop_next(c)) == NULL) {
    chat_client_update(c, 0);
    chat_server_update(s, 0);
    printf("client_pop_next_blocking...\n");
  }
  return msg;
}
...
static void test_basic(void) {
  ...
  unit_check(chat_client_connect(c1, make_addr_str(port)) == 0, "connect");
...
static void test_big_messages(void) {
  ...
  int rc;
  struct chat_message *msg;
  int count = 3;  // for fast testing (was 100)
  int real_count = 0;
  for (int i = 0; i < count; ++i) {
    unit_fail_if(chat_client_feed(c1, test_msg->data, test_msg->size) != 0);
...
static void test_multi_client(void) {
  ...
  uint16_t port = server_get_port(s);
  int client_count = 20;
  int msg_count = 3;  // For fast testing (was 100)
  uint32_t len = 1024;
  struct test_msg *test_msg = test_msg_new(len);
  struct chat_message *msg;
  ...
  for (int i = 0; i < client_count; ++i) {
    ...
    unit_fail_if(chat_client_connect(cli, make_addr_str(port)) != 0);
    server_consume_events(s);
    clis[i] = cli;
  }
  unit_msg("Say hello");
...
static void *test_stress_worker_f(void *arg) {
  ...
  int thread_idx = __atomic_fetch_add(&ctx->thread_idx, 1, __ATOMIC_RELAXED);
  ...
  for (int i = 0; i < ctx->msg_count; ++i) {
    ...
    unit_fail_if(chat_client_feed(cli, test_msg->data, test_msg->size) != 0);
...
static void test_stress(void) {
  ...
  for (int i = 0; i < client_count; ++i) {
    int rc = pthread_create(&threads[i], NULL, test_stress_worker_f, &ctx);
    unit_fail_if(rc != 0);
  }
  int *msg_counts = calloc(client_count, sizeof(msg_counts[0]));
  struct test_msg *test_msg = test_msg_new(ctx.msg_len);
