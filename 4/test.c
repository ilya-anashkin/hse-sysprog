...
static void map_reduce_inc(struct thread_pool *p, struct thread_task **tasks,
                           int count, int *arg) {
  void *result;
  for (int i = 0; i < count; ++i) {
...
static void test_timed_join(void) {
  ...
  unit_check(thread_task_timed_join(task, 0, &result) == TPOOL_ERR_TIMEOUT,
             "timed out on 0");

  struct timespec ts1, ts2;
  clock_gettime(CLOCK_MONOTONIC, &ts1);
  unit_check(thread_task_timed_join(task, 0.1, &result) == TPOOL_ERR_TIMEOUT,
  ...
  unit_check(thread_task_timed_join(task, -10000, &result) == TPOOL_ERR_TIMEOUT,
